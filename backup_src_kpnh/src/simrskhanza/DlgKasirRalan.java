...
public final class DlgKasirRalan extends javax.swing.JDialog {
    ...
    private String aktifkanparsial="no",kamar_inap_kasir_ralan=Sequel.cariIsi("select set_jam_minimal.kamar_inap_kasir_ralan from set_jam_minimal"),caripenjab="",filter="no",bangsal=Sequel.cariIsi("select set_lokasi.kd_bangsal from set_lokasi limit 1"),nonota="",
            ...
            namadokter="",namapoli="",order="reg_periksa.no_reg desc",
    ...
    private void initComponents() {
        ...
        Chksimpel = new widget.CekBox();
        jLabel11 = new widget.Label();
        LabelJumlahKunjunganPasien = new widget.Label();
        ...
        TglSakit1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "18-07-2024" }));
        ...
        TglSakit2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "18-07-2024" }));
        ...
        jLabel9.setText("Mode  simpel :");
        jLabel9.setName("jLabel9"); // NOI18N
        jLabel9.setPreferredSize(new java.awt.Dimension(85, 23));
        panelGlass9.add(jLabel9);

        Chksimpel.setBorder(null);
        Chksimpel.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        Chksimpel.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        Chksimpel.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        Chksimpel.setName("Chksimpel"); // NOI18N
        Chksimpel.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ChksimpelActionPerformed(evt);
            }
        });
        panelGlass9.add(Chksimpel);

        jLabel11.setText("Kunj. Pasien Bulan ini :");
        jLabel11.setName("jLabel11"); // NOI18N
        jLabel11.setPreferredSize(new java.awt.Dimension(120, 30));
        panelGlass9.add(jLabel11);

        LabelJumlahKunjunganPasien.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        LabelJumlahKunjunganPasien.setText("0");
        LabelJumlahKunjunganPasien.setName("LabelJumlahKunjunganPasien"); // NOI18N
        LabelJumlahKunjunganPasien.setPreferredSize(new java.awt.Dimension(50, 30));
        panelGlass9.add(LabelJumlahKunjunganPasien);
        ...
        if(tabModekasir.getRowCount()!=0){
            ...
            if(evt.getKeyCode()==KeyEvent.VK_SPACE){
                ...
                if(Sequel.cariInteger("select no_rawat from m_status_tracer where no_rawat='"+TNoRw.getText()+"' and status not in ('Belum') ")>1){
                        ...
                        JOptionPane.showMessageDialog(null, "Maaf, Pasien Belum Datang/Belum di TTV");
                }
            }
        }
...
private void MnDataPemberianObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDataPemberianObatActionPerformed
    ...
    private void ChksimpelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChksimpelActionPerformed
           //if(Sequel.cariInteger("select count(kd_dokter) from dokter where kd_dokter='"+akses.getkode()+"' ")>0){
               if(Chksimpel.isSelected()==true){ 
                tampilkasir();
               }else{
                 tampilkasir3();
               }
             // }
    }//GEN-LAST:event_ChksimpelActionPerformed
    ...
    */
    ...
    private widget.CekBox Chksimpel;
    ...
    private widget.Label LabelJumlahKunjunganPasien;
    ...
    private void tampilkasir() {
        ...
        try{   
            ...
            pskasir=koneksi.prepareStatement("select reg_periksa.no_reg,reg_periksa.no_rawat,reg_periksa.tgl_registrasi,reg_periksa.jam_reg,"+
                ...
                "reg_periksa.status_bayar,reg_periksa.status_poli,reg_periksa.kd_pj,reg_periksa.kd_poli,pasien.no_tlp "+
                "from reg_periksa inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli inner join penjab on reg_periksa.kd_pj=penjab.kd_pj inner join pemeriksaan_ralan on pemeriksaan_ralan.no_rawat=reg_periksa.no_rawat where  "+
                "reg_periksa.tgl_registrasi between ? and ? and reg_periksa.status_lanjut='Ralan' "+tampildiagnosa+
                (semua?"":"and reg_periksa.kd_pj like ? and poliklinik.nm_poli like ? and dokter.nm_dokter like ? and reg_periksa.stts like ? and reg_periksa.status_bayar like ? and "+
                "(reg_periksa.no_reg like ? or reg_periksa.no_rawat like ? or reg_periksa.tgl_registrasi like ? or reg_periksa.kd_dokter like ? or dokter.nm_dokter like ? or reg_periksa.no_rkm_medis like ? or pasien.nm_pasien like ? or poliklinik.nm_poli like ? or "+
                "reg_periksa.p_jawab like ? or penjab.png_jawab like ? or reg_periksa.almt_pj like ? or reg_periksa.status_bayar like ? or reg_periksa.hubunganpj like ?)")+
                "order by reg_periksa.no_reg asc limit 1 ");
            try{
                pskasir.setString(1,Valid.SetTgl(DTPCari1.getSelectedItem()+""));
                pskasir.setString(2,Valid.SetTgl(DTPCari2.getSelectedItem()+""));
                if(!semua){
    ...
    private void getDatakasir() {
        if(tbKasirRalan.getSelectedRow()!= -1){
            ...
            LabelJumlahKunjunganPasien.setText(Sequel.cariIsi("select count(no_rawat) from reg_periksa where no_rkm_medis ='"+TNoRMCari.getText()+"' and tgl_registrasi like '%"+DTPCari1.getSelectedItem().toString().substring(6,10)+"-"+DTPCari1.getSelectedItem().toString().substring(3,5)+"-%' "));
        }
    }
    ...
    public void isCek(){
        ...
        if(Sequel.cariInteger("select count(kd_dokter) from dokter where kd_dokter='"+akses.getkode()+"' ")>0){
                Chksimpel.setSelected(true);
            }else{
                Chksimpel.setSelected(false);
        }
