...
private void BtnGudangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnGudangActionPerformed
        ...
        if(nmgudang.getText().trim().equals("")||kdgudang.getText().trim().equals("")){
            ...
            try{  
                ...
                pstampil=koneksi.prepareStatement("select databarang.kode_brng, databarang.nama_brng,jenis.nama, databarang.kode_sat, "+
                        ...
                        " where databarang.kode_brng not in (select kode_brng from opname where tanggal like ? and kd_bangsal=?) and databarang.status='1' and databarang.kode_brng like ? or "+
                        " databarang.kode_brng not in (select kode_brng from opname where tanggal like ? and kd_bangsal=?) and databarang.status='1' and databarang.nama_brng like ? or "+
                        " databarang.kode_brng not in (select kode_brng from opname where tanggal like ? and kd_bangsal=?) and databarang.status='1' and databarang.kode_sat like ? or "+
                        " databarang.kode_brng not in (select kode_brng from opname where tanggal like ? and kd_bangsal=?) and databarang.status='1' and jenis.nama like ? "+order);
                try {
                    pstampil.setString(1,"%"+Valid.SetBln(Tgl.getSelectedItem()+"")+"%");
                    pstampil.setString(2,kdgudang.getText());
                    pstampil.setString(3,"%"+TCari.getText().trim()+"%");
                    pstampil.setString(4,"%"+Valid.SetBln(Tgl.getSelectedItem()+"")+"%");
                    pstampil.setString(5,kdgudang.getText());
                    pstampil.setString(6,"%"+TCari.getText().trim()+"%");
                    pstampil.setString(7,"%"+Valid.SetBln(Tgl.getSelectedItem()+"")+"%");
                    pstampil.setString(8,kdgudang.getText());
                    pstampil.setString(9,"%"+TCari.getText().trim()+"%");
                    pstampil.setString(10,"%"+Valid.SetBln(Tgl.getSelectedItem()+"")+"%");
    ...
    */
    ...
    public void isCek(){
         ...
         BtnTambah.setEnabled(akses.getkode()=="Admin Utama");
