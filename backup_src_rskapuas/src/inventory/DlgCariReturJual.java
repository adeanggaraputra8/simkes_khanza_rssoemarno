  ...
  if(tbRetur.getValueAt(tbRetur.getSelectedRow(),0).toString().trim().equals("")){
    ...
    private void ppCetakNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppCetakNotaActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            Valid.panggilUrl("billing/NotaApotek9.php?noretur="+NoRetur.getText()+"&nm_member="+nmmem.getText().replaceAll(" ","_")+"&tanggal="+Valid.SetTgl(TglRetur.getSelectedItem()+"")+"&petugas="+Nmptg.getText().replaceAll(" ","_"));
        }
        this.setCursor(Cursor.getDefaultCursor());
    ...
    private void tampil() {
     ...
     private void getdata() {
        if(tbRetur.getSelectedRow()!= -1){
            ...
            Sequel.cariIsi("SELECT nip FROM returjual WHERE no_retur_jual=?",Kdptg,NoRetur.getText());
            Sequel.cariIsi("SELECT petugas.nama FROM returjual INNER JOIN petugas ON petugas.nip=returjual.nip WHERE returjual.no_retur_jual=?",Nmptg,NoRetur.getText());
            Valid.SetTgl(TglRetur,tbRetur.getValueAt(tbRetur.getSelectedRow(),1).toString());
            Sequel.cariIsi("SELECT pasien.nm_pasien FROM returjual INNER JOIN pasien ON pasien.no_rkm_medis=returjual.no_rkm_medis WHERE returjual.no_retur_jual=?",nmmem,NoRetur.getText());
        }
    }
