...
public class DlgCariPermintaan extends javax.swing.JDialog {
    ...
    private PreparedStatement ps,ps2,pspeg;
    private ResultSet rs,rs2,rspeg;
     private String obat="";
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("nip")+" "+rs.getString("nama"),rs.getString("tujuan")+" ( "+rs.getString("status")+" ) "+Sequel.cariIsi("SELECT DISTINCT concat('Oleh ',nama) FROM pegawai p INNER JOIN mutasibarang m on m.nip=p.nik WHERE m.no_permintaan='"+rs.getString("no_permintaan")+"' ")
                    });  
                    ...
                    tabMode.addRow(new Object[]{"","---------------Obat/Alkes yang Disetuji--------------","","",""});
                    try{
                        pspeg=koneksi.prepareStatement("SELECT d.kode_brng,d.nama_brng,m.jml,k.satuan FROM mutasibarang m INNER JOIN permintaan_medis p ON p.no_permintaan=m.no_permintaan INNER JOIN databarang d on d.kode_brng=m.kode_brng inner join kodesatuan k on d.kode_sat=k.kode_sat WHERE m.no_permintaan=? ");

                        try {
                            pspeg.setString(1,rs.getString(2));
                            rspeg=pspeg.executeQuery();
                            int no=1;
                            while(rspeg.next()){
                                        tabMode.addRow(new Object[]{
                                        "",no+". "+rspeg.getString(1)+" "+rspeg.getString(2),rspeg.getString(3),rspeg.getString(4),""
                                    }); 
                                    no++; 
                                    }
                             } catch (Exception e) {
