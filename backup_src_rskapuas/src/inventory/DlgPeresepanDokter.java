...
//        });
    ...
    private void initComponents() {
        ...
        retriksi.setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 1, 1, 1));
        retriksi.setColumns(20);
        retriksi.setRows(5);
        retriksi.setName("retriksi"); // NOI18N
        retriksi.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                retriksiKeyPressed(evt);
            }
        });
        ...
        ChkTampilObat.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        ChkTampilObat.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        ChkTampilObat.setName("ChkTampilObat"); // NOI18N
        ChkTampilObat.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(java.awt.event.ItemEvent evt) {
                ChkTampilObatItemStateChanged(evt);
            }
        });
    ...
    private void tbResepMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbResepMouseClicked
        if(tbResep.getRowCount()!=0){
            try {
                getCekStok();
                 retriksi.setText(Sequel.cariIsi("select retriksi from databarang where kode_brng='"+tbResep.getValueAt(tbResep.getSelectedRow(),3).toString()+"' "));
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
    ...
    if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
            ...
            TCari.requestFocus();
                                    ...
                                    int reply = JOptionPane.showConfirmDialog(rootPane,"Eeiiiiiits, ada aturan pakai yang belum diisi, apa data yang mau disimpan..??","Konfirmasi",JOptionPane.YES_NO_OPTION);
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    private void retriksiKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_retriksiKeyPressed

    }//GEN-LAST:event_retriksiKeyPressed

    private void ChkTampilObatItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_ChkTampilObatItemStateChanged
    ...
    */
    ...
    private widget.TextArea retriksi;
    ...
    public void isCek(){   
        ...
        TCari.requestFocus();
        if(akses.getkode().equals("Admin Utama")||Sequel.cariInteger("select count(no_rawat) from rujukan_internal_poli where no_rawat='"+TNoRw.getText()+"' ")>0){
            ChkTampilObat.setVisible(true);
            ChkTampilObat.setSelected(true);
        }else{
            ChkTampilObat.setVisible(false);
        }
    ...
    public void setNoRm(String norwt,Date tanggal, String jam,String menit,String detik,String KodeDokter,String NamaDokter,String status) {        
        ...
        if(Sequel.cariInteger("select count(kd_dokter) from rujukan_internal_poli where kd_dokter='"+akses.getkode()+"' and no_rawat='"+TNoRw.getText()+"' ")>0){
            KdDokter.setText(akses.getkode());
            NmDokter.setText(Sequel.cariIsi("select nm_dokter from dokter where kd_dokter='"+akses.getkode()+"' "));
        }else{
    ...
    private void getCekAturanPakai() {
            if(tbResep.getSelectedRow()!=-1){
                for(r=0;r<tbResep.getRowCount();r++){
                if(Valid.SetAngka(tbResep.getValueAt(r,1).toString())>0){
                     try {
                         if(tbResep.getValueAt(r,2).toString().equals("")){
                                  JOptionPane.showMessageDialog(null,"Maaf,Ada aturan Pakai Obat yg belum dimasukan...!!!");
                         }    
                         } catch (Exception e) {
                             }
