 ...
 * @author dosen
 */
public final class DlgCariObatMapingDokter extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgCariObatMapingDokter(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        KdDokter.setBounds(90, 20, 70, 23);

        NmDokter.setEditable(false);
        NmDokter.setName("NmDokter"); // NOI18N
        FormInput.add(NmDokter);
        NmDokter.setBounds(160, 20, 190, 23);
        ...
        btnDokter.setBounds(360, 20, 28, 23);
            ...
            if (reply == JOptionPane.YES_OPTION) {
                try {  
                    ...
                    for(i=0;i<tbObat.getRowCount();i++){                        
                            if(tbObat.getValueAt(i,0).toString().equals("true")){                             
                                try {
                                        if(Sequel.menyimpantf2("maping_obat_dokter","?,?,?","Mapping Obat",3,new String[]{
                                            tbObat.getValueAt(i,1).toString(),KdDokter.getText(),NmDokter.getText()
                                        })==true){
                                          emptTeksobat();
                                        }else{
                                            sukses=false;
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariObatMapingDokter dialog = new DlgCariObatMapingDokter(new javax.swing.JFrame(), true);
    ...
    public void tampilobat() {        
        ...
        try {
                    ...
                    psobat=koneksi.prepareStatement(
                        sql+" where databarang.kode_brng not IN (SELECT kode_brng  FROM maping_obat_dokter mop WHERE mop.kd_dokter=? ) and databarang.kode_brng like ? or "+
                        " databarang.status='1' and databarang.kode_brng not IN (SELECT kode_brng  FROM maping_obat_dokter mop WHERE mop.kd_dokter=? ) and databarang.nama_brng like ? or "+
                        " databarang.status='1' and databarang.kode_brng not IN (SELECT kode_brng  FROM maping_obat_dokter mop WHERE mop.kd_dokter=? ) and kategori_barang.nama like ? or "+
                        " databarang.status='1' and databarang.kode_brng not IN (SELECT kode_brng  FROM maping_obat_dokter mop WHERE mop.kd_dokter=? ) and golongan_barang.nama like ? or "+
                        " databarang.status='1' and databarang.kode_brng not IN (SELECT kode_brng  FROM maping_obat_dokter mop WHERE mop.kd_dokter=? ) and jenis.nama like ? order by databarang.nama_brng");
                    try{
                            psobat.setString(1,KdDokter.getText());
                            psobat.setString(2,"%"+TCari.getText().trim()+"%");
                            psobat.setString(3,KdDokter.getText());
                            psobat.setString(4,"%"+TCari.getText().trim()+"%");
                            psobat.setString(5,KdDokter.getText());
                            psobat.setString(6,"%"+TCari.getText().trim()+"%");
                            psobat.setString(7,KdDokter.getText());
                            psobat.setString(8,"%"+TCari.getText().trim()+"%");
                            psobat.setString(9,KdDokter.getText());
