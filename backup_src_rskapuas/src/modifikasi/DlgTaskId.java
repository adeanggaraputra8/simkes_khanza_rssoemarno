 ...
 * @author dosen
 */
public final class DlgTaskId extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgTaskId(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
            ...
            try {
             datajam=Sequel.cariIsi("SELECT tanggal4  FROM  antrian_bantu ab  WHERE  no_rawat=? AND  tanggal4 <>'0000-00-00 00:00:00'",tbJnsPerawatan.getValueAt(i,0).toString());
                if(!datajam.equals("")){
                  ...
                  Sequel.mengedit("antrian_bantu","no_rawat='"+tbJnsPerawatan.getValueAt(i,0).toString()+"'","status='Sudah Valid',tanggal6=tanggal5 + INTERVAL "+Sequel.cariIsi("SELECT FLOOR(RAND()*(5-30+1))+30")+" MINUTE ");
                }
             }catch(Exception e){
            System.out.println("Notifikasi : "+e);
        }
            ...
            try {
             datajam=Sequel.cariIsi("SELECT tanggal5  FROM  antrian_bantu ab  WHERE  no_rawat=? AND  tanggal5 <>'0000-00-00 00:00:00'",tbJnsPerawatan.getValueAt(i,0).toString());
                if(!datajam.equals("")){
                  ...
                  Sequel.mengedit("antrian_bantu","no_rawat='"+tbJnsPerawatan.getValueAt(i,0).toString()+"'","status='Sudah Valid',tanggal7=tanggal6 + INTERVAL "+Sequel.cariIsi("SELECT FLOOR(RAND()*(1-5+1))+5")+" MINUTE ");    
                }
             }catch(Exception e){
            System.out.println("Notifikasi : "+e);
        }
