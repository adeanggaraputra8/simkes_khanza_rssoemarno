    ...
    private void BtnSimpanUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanUpdateActionPerformed
        if(NmDokterUpdate.getText().trim().equals("")){
            ...
            try {
                ...
                utc=String.valueOf(api2.GetUTCdatetimeAsString());
                headers.add("x-timestamp",utc);
                headers.add("x-signature",api2.getHmac(utc));
                ...
                System.out.println(api2.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                root = mapper.readTree(api2.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                nameNode = root.path("metadata"); 
                if(nameNode.path("code").asText().equals("200")){
                    //tampil();
                    JOptionPane.showMessageDialog(null,"Silahkan cek pada black box simrs untuk melihat data berhasil di simpan...!");
                }   else {
                    JOptionPane.showMessageDialog(null,"Silahkan cek pada black box simrs untuk melihat data berhasil di simpan...!");
                }   
            } catch (Exception ex) {
