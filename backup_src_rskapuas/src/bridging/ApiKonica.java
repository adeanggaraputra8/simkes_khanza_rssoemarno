...
public class ApiKonica {
    ...
    public void kirimRalan(String nopermintaan) {
        ...
        while (rs.next()) {
            ...
            requestJson = "{"
                + "\"acc_number[0]\":\"" + rs.getString("noorder") + "\","
                + "\"norm\":\"" + rs.getString("no_rkm_medis") + "\","
                + "\"nama_pasien\":\"" + rs.getString("nm_pasien") + "\","
                + "\"tgl_lahir\":\"" + rs.getString("tgl_lahir") + "\","
                + "\"jk\":\"" + rs.getString("jk").replaceAll("L", "M").replaceAll("P", "F") + "\","
                + "\"alamat\":\"" + rs.getString("alamat") + "\","
                + "\"kota\":\"\","
                + "\"nama_dokter\":\"" + rs.getString("nm_dokter") + "\","
                + "\"nama_pemeriksaan[0]\":\"" + rs.getString("nm_perawatan") + "\","
                + "\"modality[0]\":\"CR\","
                + "\"tgl_periksa\":\"" + rs.getString("tgl_permintaan") + " " + rs.getString("jam_permintaan") + "\","
                + "\"nama_ruangan\":\"" + rs.getString("nm_poli") + "\","
                + "\"nama_asuransi\":\"" + rs.getString("png_jawab") + "\","
                + "\"urgensi\":\"" + (rs.getString("informasi_tambahan").toLowerCase().contains("cito") ? "Cito" : "") + "\""
                + "}";

            System.out.println("JSON : " + requestJson);
            System.out.println("URL : http://192.168.2.62/api/order/add");

            requestEntity = new HttpEntity(requestJson, headers);
            stringbalik = getRest().exchange("http://192.168.2.62/api/order/add", HttpMethod.POST, requestEntity, String.class).getBody();
            JOptionPane.showMessageDialog(null, stringbalik);
        }
    } catch (Exception e) {
        System.out.println("Notif : " + e);
        if (e.toString().contains("UnknownHostException") || e.toString().contains("404")) {
            JOptionPane.showMessageDialog(null, "Koneksi ke server RIS terputus...!");
        }
    } finally {
        try {
            if (rs != null) rs.close();
