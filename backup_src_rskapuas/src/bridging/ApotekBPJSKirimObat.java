...
//    private double[] jumlahhari;
    ...
    private String no_apotek="",signa1="1",utc="",pesan="",link=koneksiDB.URLAPIAPOTEKBPJS(),kodeppkapotek=koneksiDB.KODEPPKAPOTEKBPJS(),signa2="1",nokunjungan="",kdObatSK="",requestJson="",URL="",otorisasi,sql="",aktifpcare="no",no_batchcari="", tgl_kadaluarsacari="", no_fakturcari="", aktifkanbatch="no",kodedokter="",namadokter="",noresep="",bangsal="",bangsaldefault=Sequel.cariIsi("select set_lokasi.kd_bangsal from set_lokasi limit 1"),tampilkan_ppnobat_ralan="",
        Suspen_Piutang_Obat_Ralan="",hasil="",Obat_Ralan="",HPP_Obat_Rawat_Jalan="",Persediaan_Obat_Rawat_Jalan="",hppfarmasi="",VALIDASIULANGBERIOBAT="",DEPOAKTIFOBAT="",NORESEPAKTIF="no",diagnosa_pasien, no_claim,ObatRutin,
    ...
    public ApotekBPJSKirimObat(java.awt.Frame parent, boolean modal) {
        ...
        tabModeobat=new DefaultTableModel(null,new Object[]{
                "K","Jumlah","Kode Barang","Nama Barang","Signa 1","Signa 2","Jumlah Hari","Jumlah 2"
            }){
        ...
        carisep.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if(carisep.getTable().getSelectedRow()!= -1){
                    NoSEP.setText(carisep.getTable().getValueAt(carisep.getTable().getSelectedRow(),1).toString());
                    KdPoli.setText(carisep.getTable().getValueAt(carisep.getTable().getSelectedRow(),15).toString()); 
                    NmPoli.setText(carisep.getTable().getValueAt(carisep.getTable().getSelectedRow(),16).toString());
                    KdDPJP.setText(carisep.getTable().getValueAt(carisep.getTable().getSelectedRow(),17).toString());
                    NmDPJP.setText(carisep.getTable().getValueAt(carisep.getTable().getSelectedRow(),18).toString());
...
//            NORESEPAKTIF=koneksiDB.NORESEPAKTIF();
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Data Obat Apotek BPJS ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Helvetica Neue", 0, 13), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        DTPTgl.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "28-06-2026" }));
        ...
        TanggalPelayanan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "28-06-2026" }));
        ...
        Iterasi.setBounds(610, 100, 130, 24);
        ...
        jLabel4.setBounds(650, 10, 70, 23);
        ...
        TInfoPRB.setBounds(720, 10, 200, 23);
        ...
        jLabel6.setBounds(640, 40, 80, 23);
        ...
        TInfoIterasi.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                TInfoIterasiActionPerformed(evt);
            }
        });
        FormInput.add(TInfoIterasi);
        TInfoIterasi.setBounds(720, 40, 120, 23);

        btnPilihSep.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/190.png"))); // NOI18N
        btnPilihSep.setMnemonic('3');
        btnPilihSep.setText("Cari SEP");
        btnPilihSep.setToolTipText("Alt+3");
        btnPilihSep.setName("btnPilihSep"); // NOI18N
        btnPilihSep.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnPilihSepActionPerformed(evt);
            }
        });
        btnPilihSep.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                btnPilihSepKeyPressed(evt);
            }
        });
    ...
    private void tbObatMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbObatMouseClicked
        if(tbObat.getRowCount()!=0){
            try {
                if(tbObat.getSelectedColumn()==5||tbObat.getSelectedColumn()==4){
                   HitungKapasitas(tbObat.getSelectedRow());
                    getDataobat();
                }
    ...
    private void tbObatKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbObatKeyPressed
        if(tbObat.getRowCount()!=0){
            if(evt.getKeyCode()==KeyEvent.VK_ENTER){
                try {
                 if(tbObat.getSelectedColumn()==6||tbObat.getSelectedColumn()==4||tbObat.getSelectedColumn()==5){
                    HitungKapasitas(tbObat.getSelectedRow());
                    getDataobat();
                }
                } catch (java.lang.NullPointerException e) {
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
    ...
    }else if(jml>0 && TabRawat.getSelectedIndex()==0 || TabRawat.getSelectedIndex()==1){     
    ...
    //                            racikan
                                ...
                                for(i=0;i<tbDetailObatRacikan.getRowCount();i++){ 
                                    if(Valid.SetAngka(tbDetailObatRacikan.getValueAt(i,1).toString())>0){
                                        if(!tbDetailObatRacikan.getValueAt(i,7).toString().equals("")){
                                        try {
                                            requestJson = "{\n"
                                                    + "            \"NOSJP\": \"" + no_apotek + "\",\n"
                                                    + "            \"NORESEP\": \"" + TResep.getText() + "\",\n"
                                            ...
                                            if (ex.toString().contains("UnknownHostException")) {
                                              ...
                                              try {
                                                ...
                                                System.out.println("Detail Obat : "+requestJson);                                            
                                                requestEntity = new HttpEntity(requestJson, headers);
                                                root = mapper.readTree(api.getRest().exchange(link + "/obatnonracikan/v3/insert", HttpMethod.POST, requestEntity, String.class).getBody());
...
//                                    + "\"TGLSJP\": \"" + Valid.SetTgl(DTPTgl.getSelectedItem()+"")+" "+Jam.getText()+ "\","+
    ...
    private void TInfoIterasiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_TInfoIterasiActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_TInfoIterasiActionPerformed

    private void btnPilihSepActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPilihSepActionPerformed
        carisep.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        carisep.isCek(NoKartu.getText());
        carisep.setLocationRelativeTo(internalFrame1);
        carisep.setVisible(true);
    }//GEN-LAST:event_btnPilihSepActionPerformed

    private void btnPilihSepKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_btnPilihSepKeyPressed
                        ...
                        while(rsobat.next()){
                            if (rsobat.getString("jml").equals("15")) {
                              ...
                              tabModeobat.addRow(new Object[] {false,rsobat.getString("jml"),rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),1,1,rsobat.getString("jml"),rsobat.getString("jml")});
                                ...
                                tabModeobat.addRow(new Object[] {false,rsobat.getString("jml"),rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),1,1,23,rsobat.getString("jml")}); 
                            } else if (rsobat.getString("jml").equals("45")) {
                                tabModeobat.addRow(new Object[] {false,rsobat.getString("jml"),rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),1,1.5,30,rsobat.getString("jml")}); 
                            } else if (rsobat.getString("jml").equals("60")) {
                                tabModeobat.addRow(new Object[] {false,rsobat.getString("jml"),rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),2,1,30,rsobat.getString("jml")}); 
                            } else if (rsobat.getString("jml").equals("90")) {
                                tabModeobat.addRow(new Object[] {false,rsobat.getString("jml"),rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),3,1,30,rsobat.getString("jml")}); 
                            } else if (rsobat.getString("jml").equals("120")) {
                                tabModeobat.addRow(new Object[] {false,rsobat.getString("jml"),rsobat.getString("kode_brng"),rsobat.getString("nama_brng"),4,1,30,rsobat.getString("jml")}); 
                            } else {
...
//                 System.out.println("Notifikasi racikan : "+e);
    ...
    public void setNoRm(String norwt,String norm,String nama,String tanggal, String jam, String Resep, String Nresep) {        
        ...
        Lahir.setText(Sequel.cariIsi("select tgl_lahir from pasien where no_rkm_medis =?",TNoRM.getText()));
        ...
        TInfoIterasi.setText(Sequel.cariIsi("select if(iterasi=1,'Iter 1 kali',if(iterasi=2, 'Iter 2 Kali','')) from flagging_iterasi where no_resep='"+Nresep+"' "));
        switch (Sequel.cariIsi("select iterasi from flagging_iterasi where no_resep ='"+Nresep+"' ")) {
                    case "1":
...
//                                        response.path("noSep_Kunjungan").asText(),
                ...
                }else if (!Sequel.cariIsi("select no_apotek from bridging_apotek_bpjs where no_resep='" + TResep.getText() + "' and no_sep='"+NoSEP.getText()+"' ").isEmpty() && Iterasi.getSelectedIndex() == 2) {
                    URL = link + "/obatracikan/v3/insert";
                    System.out.println(URL);
                    for(i=0;i<tbDetailObatRacikan.getRowCount();i++){ 
                        if(Valid.SetAngka(tbDetailObatRacikan.getValueAt(i,1).toString())>0){
...
//                                        response.path("noSep_Kunjungan").asText(),
     ...
     private void HitungKapasitas(int jmlh){        
          ...
          //  if(jmlh==1||jmlh==2||jmlh==3||jmlh==4||jmlh==5||jmlh==6||jmlh==7){
                try {
                    if(tbObat.getValueAt(jmlh,7).toString().equals("1")||tbObat.getValueAt(jmlh,7).toString().equals("2")||tbObat.getValueAt(jmlh,7).toString().equals("3")||tbObat.getValueAt(jmlh,7).toString().equals("4")||tbObat.getValueAt(jmlh,7).toString().equals("5")||tbObat.getValueAt(jmlh,7).toString().equals("6")||tbObat.getValueAt(jmlh,7).toString().equals("7")){   
                        tbObat.setValueAt(Valid.SetAngka2(Double.parseDouble(tbObat.getValueAt(jmlh,1).toString())/Double.parseDouble(tbObat.getValueAt(jmlh,4).toString())), jmlh,6);     
                    } 
                } catch (Exception e) {
