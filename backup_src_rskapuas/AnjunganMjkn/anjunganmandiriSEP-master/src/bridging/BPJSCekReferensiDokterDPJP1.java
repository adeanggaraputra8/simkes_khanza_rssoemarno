...
public final class BPJSCekReferensiDokterDPJP1 extends javax.swing.JDialog {
    ...
    public BPJSCekReferensiDokterDPJP1(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"No.", "Kode Dokter", "Nama Dokter","Poliklinik"}) {
            @Override
                ...
                if (response.path("list").isArray()) {
                    ...
                    for (JsonNode list : response.path("list")) {
                        if (list.path("kode").asText().toLowerCase().contains(poli.toLowerCase())
                                || list.path("nama").asText().toLowerCase().contains(poli.toLowerCase())) {
                            tabMode.addRow(new Object[]{
                                i + ".", list.path("kode").asText(), list.path("nama").asText(),Sequel.cariIsi("SELECT p.nm_poli FROM poliklinik p INNER JOIN jadwal j on j.kd_poli=p.kd_poli INNER JOIN maping_dokter_dpjpvclaim mdd ON mdd.kd_dokter=j.kd_dokter WHERE mdd.kd_dokter_bpjs='"+list.path("kode").asText()+"' GROUP BY nm_poli")
                            });
                        }
                        i++;
                    }
