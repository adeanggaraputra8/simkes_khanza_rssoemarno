...
//            JOptionPane.showMessageDialog(rootPane, "Maaf, dokter  tidak berpraktek pada tanggal yang anda pilih ");
        } else if (Sequel.cariInteger("select count(no_rkm_medis) from reg_periksa where kd_pj='002' and no_rkm_medis='" + lblNoRM.getText() + "' and kd_poli='" + kode_poli + "' and kd_dokter='" + kode_dokter + "' and tgl_registrasi='" + Valid.SetTgl(TanggalPeriksa.getSelectedItem() + "") + "' ") > 0) {
            ...
            if (Sequel.menyimpantf2("reg_periksa", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rawat", 19,
                    new String[]{NoReg.getText(), NoRawat.getText(), Valid.SetTgl(TanggalPeriksa.getSelectedItem() + ""), Sequel.cariIsi("select current_time()"),
                        ...
                        "Lama", "Ralan","002", umur, sttsumur, "Belum Bayar", status}) == true) {
...
//            tampilPoli();
    ...
    private void tampilPenjab() {
        try {
            ...
            iyem = "";
            ps = koneksi.prepareStatement("select * from penjab where status='1' AND kd_pj='002' order by kd_pj");
...
//    }
    ...
    private void MnCetakRegisterActionPerformed(String norawat) {
        ...
        Map<String, Object> param = new HashMap<>();
            ...
            "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.tgl_registrasi,poliklinik.nm_poli,penjab.png_jawab,reg_periksa.no_reg,pasien.jk " +
