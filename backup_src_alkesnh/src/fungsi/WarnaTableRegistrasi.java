...
public class WarnaTableRegistrasi extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column){
       ...
       if (row % 2 == 1){
        ...
        if(Sequel.cariInteger("SELECT count(p.no_rkm_medis) FROM pasien p INNER JOIN status_pasien sp ON sp.no_rm=p.no_rkm_medis WHERE p.no_rkm_medis IN (SELECT rp.no_rkm_medis FROM reg_periksa rp INNER JOIN pemeriksaan_ralan pr ON rp.no_rawat=pr.no_rawat WHERE SUBSTRING(pr.tensi,1,3) > 159 ) AND sp.status = '-' and p.no_rkm_medis ='"+table.getValueAt(row,7).toString()+"' ")>0 && table.getValueAt(row,24).toString().trim().equals("-") ){
           component.setBackground(Color.RED);
           component.setForeground(Color.BLACK);
        }else if (table.getValueAt(row,24).toString().trim().equals("Prolanis")){
            component.setBackground(Color.GREEN);
           component.setForeground(Color.BLACK);
        }else if(table.getValueAt(row,24).toString().trim().equals("PRB")){
