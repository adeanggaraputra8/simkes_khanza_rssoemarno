...
import rekammedis.RMCariKeluhan;
import rekammedis.RMCariPemeriksaan;
...
public final class DlgOdontogram extends javax.swing.JDialog {
    ...
    private String bagian,RESEPRAJALKEPLAN="no",resep="",status="",diagnosa="";
    ...
    public DlgOdontogram(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            ...
            "Hasil Pemeriksaan","Tanggal Permintaan","No Permintaan"
        }){
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Pemeriksaan Odontogram ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog", 1, 12), new java.awt.Color(50, 50, 50))); // NOI18N
...
//        }else{
            if(Sequel.menyimpantf("pemeriksaan_gigi","?,?,?,?,?,?,?,?","No.Rawat",8,new String[]{
                    TNoRw.getText(),KodeDokter.getText(),bagian,DiagnosaUtama.getText(),KodeDiagnosaUtama.getText(),
                    Hasil.getSelectedItem().toString(),Valid.SetTgl(Tanggal.getSelectedItem()+""),
                       ...
                       if(RESEPRAJALKEPLAN.equals("yes")){
                                                    try {
                                                        ...
                                                        try {
                                                            ...
                                                            ps.setString(2,KodeDokter.getText());
                                                            rs=ps.executeQuery();
                                                            if(rs.next()){
                                                                resep="\n Hasil Odontogram : \n";
                                                                diagnosa="\n Diagnosa : \n";
                                                                psresep=koneksi.prepareStatement(
                                                                       "SELECT pg.bagian,concat(pg.diagnosa,' ',pg.kd_diagnosa) diagnosa,pg.hasil FROM  pemeriksaan_gigi pg WHERE pg.no_rawat=? ");
                                                                try {
                                                                    psresep.setString(1,TNoRw.getText());
                                                                    rsobat=psresep.executeQuery();
                                                                    while(rsobat.next()){
                                                                        resep=resep+rsobat.getString("bagian")+" Hasil "+rsobat.getString("hasil")+"\n";
                                                                        diagnosa=diagnosa+rsobat.getString("diagnosa")+"\n";
                                                                    }
                                                                } catch (Exception e) {
                                                                ...
                                                                Sequel.queryu2("update pemeriksaan_ralan set rtl=concat(rtl,' ',?),penilaian=concat(penilaian,' ',?) where no_rawat=? and tgl_perawatan=? and jam_rawat=? and nip=?",6,new String[]{
                                                                    resep,diagnosa,TNoRw.getText(),rs.getString("tgl_perawatan"),rs.getString("jam_rawat"),KodeDokter.getText()
                                                                });
                                                            }
                                                        } catch (Exception e) {
                                                            System.out.println("Notif : "+e);
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        ...
        if(gigi85.isSelected()==true){
            ...
            if(tbObat.getSelectedRow()>-1){
                if(Sequel.mengedittf("pemeriksaan_gigi","no_rawat=?","no_rawat=?,kd_dokter=?,bagian=?,diagnosa=?,kd_diagnosa=?,hasil=?,tanggal=?",8,new String[]{
                    TNoRw.getText(),KodeDokter.getText(),bagian,DiagnosaUtama.getText(),KodeDiagnosaUtama.getText(),
                    Hasil.getSelectedItem().toString(),Valid.SetTgl(Tanggal.getSelectedItem()+""),
    ...
    private void HasilKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_HasilKeyPressed
        Valid.pindah(evt,Hasil,BtnSimpan);
        ...
        try{
            if(TCari.getText().equals("")){
                ps=koneksi.prepareStatement(
                    ...
                    "pemeriksaan_gigi.hasil,pemeriksaan_gigi.tanggal,pemeriksaan_gigi.noorder from pemeriksaan_gigi inner join reg_periksa on pemeriksaan_gigi.no_rawat=reg_periksa.no_rawat  "+
...
//                    ps.setString(36,"%"+TCari.getText()+"%");
                ...
                while(rs.next()){
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("kd_diagnosa"),rs.getString("hasil"),rs.getString("tanggal"),rs.getString("noorder")
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
...
//        gigi85.setSelected(true);
    ...
    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            ...
            Valid.SetTgl2(Tanggal,tbObat.getValueAt(tbObat.getSelectedRow(),11).toString());
            TNoPermintaan.setText(tbObat.getValueAt(tbObat.getSelectedRow(),12).toString());
