...
import modifikasi.DlgRekapPenjualanObatNilaiBarang;
...
public class DlgCariPenjualan extends javax.swing.JDialog {
    ...
    private DlgRekapPenjualanObatNilaiBarang nb = new DlgRekapPenjualanObatNilaiBarang(null, false);
    ...
    private void initComponents() {
        ...
        ppNilaiTransaksii = new javax.swing.JMenuItem();
        ...
        ppNilaiTransaksii.setBackground(new java.awt.Color(255, 255, 254));
        ppNilaiTransaksii.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppNilaiTransaksii.setForeground(new java.awt.Color(50, 50, 50));
        ppNilaiTransaksii.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppNilaiTransaksii.setText("Rekapan Jumlah Transaksi");
        ppNilaiTransaksii.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppNilaiTransaksii.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppNilaiTransaksii.setName("ppNilaiTransaksii"); // NOI18N
        ppNilaiTransaksii.setPreferredSize(new java.awt.Dimension(190, 25));
        ppNilaiTransaksii.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppNilaiTransaksiiActionPerformed(evt);
            }
        });
        jPopupMenu1.add(ppNilaiTransaksii);
        ...
        BtnCloseIn4.setBounds(640, 30, 100, 30);
        ...
        BtnSimpan5.setText("Simpan Semua");
        ...
        internalFrame5.add(BtnSimpan5);
        BtnSimpan5.setBounds(500, 30, 150, 30);
        ...
        label12.setText("Shift :");
        label12.setName("label12"); // NOI18N
        label12.setPreferredSize(new java.awt.Dimension(45, 23));
        panelisi1.add(label12);

        Shift.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Semua", "Pagi (14:00)", "Malam" }));
        Shift.setName("Shift"); // NOI18N
        Shift.setPreferredSize(new java.awt.Dimension(120, 23));
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    ...
    private void ppVerifActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppVerifActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
                ...
                WindowGantiCaraBayar.setSize(745,80);
    ...
    private void BtnSimpan4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpan4ActionPerformed
        if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
            ...
            try {
               pscarijual=koneksi.prepareStatement(
                       "select penjualan.nota_jual,penjualan.kd_bangsal,penjualan.status,penjualan.no_rkm_medis,penjualan.nm_pasien,SUBSTRING(tgl_jual,1,10) as tgl_jual,penjualan.ongkir,penjualan.ppn from penjualan where penjualan.nota_jual=?");
               try {
                  pscarijual.setString(1,tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim());
                  rs=pscarijual.executeQuery();
                  if(rs.next()){
      ...
      if (tabMode.getRowCount() == 0) {
            ...
            if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
                Valid.textKosong(TCari,"No.Nota");
                    ...
                    for (int j = 0; j < tbDokter.getRowCount(); j++) {
                         if (tbDokter.getValueAt(j, 0).toString().trim().equals("")) {
                             //Valid.textKosong(TCari,"No.Faktur");
                                 ...
                                 try {
                                    pscarijual=koneksi.prepareStatement(
                                            "select penjualan.nota_jual,penjualan.kd_bangsal,penjualan.status,penjualan.no_rkm_medis,penjualan.nm_pasien,SUBSTRING(tgl_jual,1,10) as tgl_jual,penjualan.ongkir,penjualan.ppn from penjualan where penjualan.nota_jual=?");
                                    try {
                                       pscarijual.setString(1, tbDokter.getValueAt(j, 0).toString().trim());
                                       rs=pscarijual.executeQuery();
                                       if(rs.next()){
                                           if(rs.getString("status").equals("Sudah Dibayar")){
                                               //JOptionPane.showMessageDialog(rootPane,"Maaf transaksi penjualan sudah diverifikasi..!!");
                                              ...
                                              kdrek="";
                                              besarppn=0;
                                              ttljual=rs.getDouble("ongkir")+rs.getDouble("ppn")+Sequel.cariIsiAngka("select sum(detailjual.total) from detailjual where detailjual.nota_jual=?",tbDokter.getValueAt(j,0).toString().trim());
                                              ...
                                              Sequel.mengedit("penjualan","nota_jual=?","kd_rek=?,nama_bayar=?",3,new String[]{
                                                  kdrek,AkunBayar.getSelectedItem().toString(),tbDokter.getValueAt(j,0).toString().trim()
                                              });
    ...
    private void ppNilaiTransaksiiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppNilaiTransaksiiActionPerformed
        nb.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
        nb.setLocationRelativeTo(internalFrame1);
        nb.setAlwaysOnTop(false);
        nb.setVisible(true);
    }//GEN-LAST:event_ppNilaiTransaksiiActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem ppNilaiTransaksii;
    ...
    private void tampil() {
        if(Shift.getSelectedIndex()==0){
              tanggal="  penjualan.tgl_jual between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+" 00:00:00' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+" 23:59:59' ";
        }else if(Shift.getSelectedIndex()==1){
                tanggal="  penjualan.tgl_jual between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+" 00:00:00' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+" 14:00:01' ";
        }else if(Shift.getSelectedIndex()==2){
                tanggal="  penjualan.tgl_jual between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+" 14:00:02' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+" 23:59:59' ";
        } 
    ...
    public void isCek(){
        ...
        ppHapus.setEnabled(akses.getpenjualan_obat()); 
         if(akses.getkode().equals("Admin Utama")){
           ppNilaiTransaksii.setEnabled(true);
         }else{
            ppNilaiTransaksii.setEnabled(false);
         }
    }
