...
//        System.out.println("Notif : "+jSisa);
 ...
 i=JOptionPane.showConfirmDialog(null, "Apakah Anda yakin ingin mereset antrian dari 0 kembali ?","Konfirmasi",JOptionPane.YES_NO_OPTION);
    ...
    private void BtnKeluar3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKeluar3ActionPerformed
        Sequel.queryu2("update tt_queue set status='Open' WHERE type='dokter' and client='"+KodeDokter.getText()+"' and queue_number='"+jNoAntrian.getText()+"' and date(datetime)='"+dateFormat.format(new Date())+"'");
//        jNoAntrian.setText(Sequel.cariIsi("SELECT queue_number FROM tt_queue WHERE type='dokter' and client='"+akses.getkode()+"' and status='Closed' ORDER BY id DESC LIMIT 1 "));
//           // TODO add your handling code here:
    ...
    private void BtnKeluar4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKeluar4ActionPerformed
       ...
       String sisa = Sequel.cariIsi("SELECT count(client) FROM tt_queue WHERE type='dokter' and client='"+KodeDokter.getText()+"' and status='Pending' and date(datetime)='"+dateFormat.format(new Date())+"' ");
       if(sisa.equals("0")){
            JOptionPane.showMessageDialog(null,"Maaf, data Antrian sudah habis.");
                  
       }else{
        Sequel.queryu2("update tt_queue set status='Open' WHERE type='dokter' and client='"+KodeDokter.getText()+"' and status='Pending' and date(datetime)='"+dateFormat.format(new Date())+"' ORDER BY id ASC LIMIT 1");
        Jumlah.setText(Sequel.cariIsi("SELECT count(client) FROM tt_queue WHERE type='dokter' and client='"+KodeDokter.getText()+"' and date(datetime)='"+dateFormat.format(new Date())+"'"));
        SisaAntrian.setText(Sequel.cariIsi("SELECT count(client) FROM tt_queue WHERE type='dokter' and client='"+KodeDokter.getText()+"' and status='Pending' and date(datetime)='"+dateFormat.format(new Date())+"' "));
        jSisa.setText(Sequel.cariIsi("SELECT count(client) FROM tt_queue WHERE type='dokter' and client='"+KodeDokter.getText()+"' and status='Pending' and date(datetime)='"+dateFormat.format(new Date())+"' "));
        jNoAntrian.setText(Sequel.cariIsi("SELECT queue_number FROM tt_queue WHERE type='dokter' and client='"+KodeDokter.getText()+"' and status='Open' and date(datetime)='"+dateFormat.format(new Date())+"' ORDER BY id DESC LIMIT 1 "));
       }        // TODO add your handling code here:
