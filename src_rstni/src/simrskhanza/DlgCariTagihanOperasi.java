...
public class DlgCariTagihanOperasi extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        MnLaporanOperasiTTE = new javax.swing.JMenuItem();
        ...
        MnLaporanOperasiTTE.setBackground(new java.awt.Color(255, 255, 254));
        MnLaporanOperasiTTE.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MnLaporanOperasiTTE.setForeground(new java.awt.Color(50, 50, 50));
        MnLaporanOperasiTTE.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnLaporanOperasiTTE.setText("Laporan Operasi Pasien TTE");
        MnLaporanOperasiTTE.setName("MnLaporanOperasiTTE"); // NOI18N
        MnLaporanOperasiTTE.setPreferredSize(new java.awt.Dimension(220, 26));
        MnLaporanOperasiTTE.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnLaporanOperasiTTEActionPerformed(evt);
            }
        });
        jPopupMenu1.add(MnLaporanOperasiTTE);
...
private void MnHapusObatOperasiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnHapusObatOperasiActionPerformed
    ...
    private void MnLaporanOperasiTTEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLaporanOperasiTTEActionPerformed
        String data ="";
        if(tbDokter.getSelectedRow()>-1){
            if(!tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString().equals("")){
                Map<String, Object> param = new HashMap<>();    
                ...
                if(Sequel.cariIsi("select reg_periksa.status_lanjut from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()).equals("Ralan")){
                  ...
                  data = Valid.saveToPDFTte("rptLaporanOperasi.jasper","report",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString(),param);
                 ttefile.isCek(data, kdmem.getText(),tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString(), Sequel.cariIsi("select operasi.operator1 from operasi where operasi.no_rawat='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()+"' and tgl_operasi='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString()+"'"));
    ...
    }//GEN-LAST:event_MnLaporanOperasiTTEActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem MnLaporanOperasiTTE;
    ...
    private void getData() {
        ...
        if(row!= -1){
            ...
            kdmem.setText(Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()+"' "));
