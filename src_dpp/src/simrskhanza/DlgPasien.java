...
public class DlgPasien extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        DTPLahir.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "15-03-2024" }));
        ...
        DTPDaftar.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "15-03-2024" }));
...
private void KabupatenMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_KabupatenMouseMoved
    ...
    private void MnKartu1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnKartu1ActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            param.put("emailrs",akses.getemailrs()); 
            param.put("logo",Sequel.cariGambar("select setting.logo from setting")); 
            Valid.MyReportqry("rptKartuBerobat9.jasper","report","::[ Kartu Periksa Pasien ]::","select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
                   "pasien.tmp_lahir, pasien.tgl_lahir, concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab,', ',propinsi.nm_prop) as alamat, pasien.gol_darah, pasien.pekerjaan,"+
            ...
            if(cmbHlm.getSelectedItem().toString().equals("Semua")){
                 if(Carialamat.getText().trim().equals("")&&TCari.getText().trim().equals("")&&tampilindikasiprolanis.equals("")){
                      ps=koneksi.prepareStatement("select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
                           ...
                           "inner join cacat_fisik on pasien.cacat_fisik=cacat_fisik.id inner join propinsi on pasien.kd_prop=propinsi.kd_prop "+
                           "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien inner join suku_bangsa on suku_bangsa.id=pasien.suku_bangsa "+
                           "inner join penjab on pasien.kd_pj=penjab.kd_pj inner join status_pasien on status_pasien.no_rm=pasien.no_rkm_medis order by pasien.tgl_daftar desc,pasien.no_rkm_medis desc");  
                 }else{
                      ps=koneksi.prepareStatement("select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
                           "pasien.tmp_lahir, pasien.tgl_lahir,pasien.nm_ibu,pasien.agama,pasien.alamat,kelurahan.nm_kel,kecamatan.nm_kec,kabupaten.nm_kab,propinsi.nm_prop,"+
                           "pasien.gol_darah, pasien.pekerjaan,pasien.stts_nikah,status_pasien.status,pasien.tgl_daftar,pasien.no_tlp,pasien.umur,"+
                           ...
                           "or pasien.no_tlp like ? or status_pasien.status like ?) order by pasien.tgl_daftar desc,pasien.no_rkm_medis desc");  
                 }    
            }else{
                if(Carialamat.getText().trim().equals("")&&TCari.getText().trim().equals("")&&tampilindikasiprolanis.equals("")){
                    ps=koneksi.prepareStatement("select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
                           ...
                           "inner join cacat_fisik on pasien.cacat_fisik=cacat_fisik.id inner join propinsi on pasien.kd_prop=propinsi.kd_prop "+
                           "inner join bahasa_pasien on bahasa_pasien.id=pasien.bahasa_pasien inner join suku_bangsa on suku_bangsa.id=pasien.suku_bangsa "+
                           "inner join penjab on pasien.kd_pj=penjab.kd_pj inner join status_pasien on status_pasien.no_rm=pasien.no_rkm_medis order by pasien.tgl_daftar desc,pasien.no_rkm_medis desc LIMIT ? ");    
                }else{
                    ps=koneksi.prepareStatement("select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
                           "pasien.tmp_lahir, pasien.tgl_lahir,pasien.nm_ibu,pasien.agama,pasien.alamat,kelurahan.nm_kel,kecamatan.nm_kec,kabupaten.nm_kab,propinsi.nm_prop,"+
                           "pasien.gol_darah, pasien.pekerjaan,pasien.stts_nikah,status_pasien.status,pasien.tgl_daftar,pasien.no_tlp,pasien.umur,"+
                           ...
                           "or pasien.stts_nikah like ? or pasien.nip like ? or cacat_fisik.nama_cacat like ? or pasien.namakeluarga like ? or perusahaan_pasien.nama_perusahaan like ? "+
                           "or bahasa_pasien.nama_bahasa like ? or suku_bangsa.nama_suku_bangsa like ? or pasien.nm_ibu like ? or pasien.tgl_daftar like ? "+
                           "or pasien.no_tlp like ? or status_pasien.status like ?)  order by pasien.tgl_daftar desc,pasien.no_rkm_medis desc LIMIT ? ");    
                }       
            }          
            try{
                if(cmbHlm.getSelectedItem().toString().equals("Semua")){
